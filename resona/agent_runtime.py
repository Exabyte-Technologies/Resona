...
For shell capabilities, an inline page script may call `parent.postMessage({{type: 'resona:audio', action: 'toggle'}}, '*')`. Supported audio actions are exactly: `toggle`; `setBeat` with numeric `value`; `setNoise` with `value` equal to white, pink, brown, rain, ocean, or forest; `toggleNoise`; `setLayer` with a layer `name` and 0-100 `value`; `setVolume` with `name` equal to binaural, ambient, or noise and 0-100 `value`; `toggleAmbient`; and `setAmbient` with `name` equal to drone, pads, textures, melody, or spatial and 0-100 `value`. Noise is strictly independent: binaural playback never creates or starts a noise source, and noise plays only after `toggleNoise` from the Noise Generator card. Prefer the automatic HTML controls `data-playback-toggle`, `data-band`, `data-noise`, `data-noise-toggle`, `data-audio`, `data-volume`, `data-ambient-toggle`, and `data-ambient` when possible. Do not invent unsupported message actions or argument names. It may request safe account data with `{{type: 'resona:request', resource: 'profile'}}` or `{{type: 'resona:request', resource: 'history'}}`. Listen for `message` events named `resona:audio-state`, `resona:profile`, or `resona:history` to receive results. Use ordinary page-local JavaScript directly for custom controls such as breathing timers, tabs, accordions, and animations. HTML closing tags must be literal, such as `</script>`; never put a backslash before `/script` in an HTML closing tag and never insert literal `\n` text in place of line breaks.
