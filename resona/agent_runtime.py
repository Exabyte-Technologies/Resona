...
For shell capabilities, an inline page script may call `parent.postMessage({{type: 'resona:audio', action: 'toggle'}}, '*')`. Supported audio actions are exactly: `toggle`; `setBeat` with numeric `value`; `setNoise` with `value` equal to white, pink, brown, rain, ocean, or forest; `toggleNoise`; `setLayer` with a layer `name` and 0-100 `value`; `setVolume` with `name` equal to binaural, ambient, or noise and 0-100 `value`; `toggleAmbient`; `setDroneFrequency` with a 40-400 Hz numeric `value`; `setChordTransition` and `setBinauralChordTransition` with a 0-4 second numeric `value`; and `setAmbient` with `name` equal to drone, pads, textures, melody, or spatial and 0-100 `value`. The browser-only chord model is exposed through `generateChordProgression` with `seedChords`, `length`, `duration` from 2-120 seconds per chord, `temperature`, `topK`, `greedy`, and optional `continuous`; continuous mode can also be changed with `setContinuousChordMode` and boolean `enabled`. Playback actions are `toggleChordProgression`, `stopChordProgression`, and `replayChordProgression`. Prefer the automatic chord controls already used by the default page: `data-chord-card`, `data-chord-seed`, `data-chord-length`, `data-chord-duration`, `data-chord-temperature`, `data-chord-top-k`, `data-chord-greedy`, `data-chord-continuous`, `data-chord-transition`, `data-binaural-chord-transition`, `data-chord-generate`, `data-chord-play`, `data-chord-stop`, and `data-chord-replay`. Continuous mode prepares two sets before playback, keeps one set ready, and generates the following set while the current one plays. The drone frequency is the base ambient harmonic root and the base binaural tonal center. Generated chords are simplified into one diatonic key and retune the pads, melodic tones, and binaural carrier together at each chord boundary using the independently configured progression transition times; zero seconds is instant. The selected binaural beat difference remains constant while its left and right frequencies move around the active chord root. Manual drone-frequency changes retain their separate smooth binaural glide and are not affected by progression transition controls. The drone remains anchored, textures stay broadband, spatial effects stay pitchless, and noise remains independent. Prefer automatic HTML controls when possible. Do not invent unsupported message actions or argument names. It may request safe account data with `{{type: 'resona:request', resource: 'profile'}}` or `{{type: 'resona:request', resource: 'history'}}`. Listen for `message` events named `resona:audio-state`, `resona:chord-result`, `resona:chord-status`, `resona:chord-pipeline`, `resona:profile`, or `resona:history` to receive results. Use ordinary page-local JavaScript directly for custom controls such as breathing timers, tabs, accordions, and animations. HTML closing tags must be literal, such as `</script>`; never put a backslash before `/script` in an HTML closing tag and never insert literal `\n` text in place of line breaks.
